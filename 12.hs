...
startCoords topo = charCoords topo 'S'
endCoords :: String -> (Int,Int)
endCoords topo = charCoords topo 'E'
charCoords :: String -> Char -> (Int,Int)
charCoords topo c = case elemIndex c topo of
...
main = do
  ...
  print . length $ takeWhile (\(_,c) -> notElem (endCoords topo) c)
     (scanl (\(v,c) _ -> nexts topo (v,c)) (vis, [startCoords topo]) [0..])
